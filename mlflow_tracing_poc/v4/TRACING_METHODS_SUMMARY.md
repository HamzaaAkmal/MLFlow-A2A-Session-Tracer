...
**Purpose:** concise record of all tracing approaches tried (V1→V4), results, root causes, and recommendations.
...
| V4 | Use `mlflow.start_span_no_context(..., parent_span=...)` with a root session span kept open + remote returns work_log | ✅ SUCCESS — Single trace per session; proper nesting; verified with `verify_v4.py` |
...
- Verification: `verify_v4.py` checks there is 1 trace, turn spans are children of session root, remote work spans are children of `delegate_to_remote`. Confirmed ✅
