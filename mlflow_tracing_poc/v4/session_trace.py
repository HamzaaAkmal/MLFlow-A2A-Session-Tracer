"""
V4: TRUE Single-Trace Solution Using start_span_no_context
==========================================================

This is the CORRECT solution for MLflow multi-turn, multi-agent tracing.

Key insight: Use `mlflow.start_span_no_context()` with `parent_span` parameter
to explicitly control parent-child relationships across turns.

Pattern:
1. Create a ROOT span for the session (kept as reference)
2. Each turn creates a child span using parent_span=root_span
3. Within each turn, nested spans use parent_span=turn_span
4. Remote agent work is logged as child spans

This ensures ALL spans in a session are in ONE trace with proper nesting.
"""
